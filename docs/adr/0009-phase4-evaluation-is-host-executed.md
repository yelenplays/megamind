...
Blinding is a three-artifact split: the plan and the grader packet never name a
condition beside a label, and the unblinding map lives with the host execution
record. Conditions are assigned to labels by a deterministic permutation of the
seed and the frozen identity, and scoring seals the blind per-label scores
