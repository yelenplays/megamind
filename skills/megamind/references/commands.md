...
`bench run --fixtures DIR --queries FILE --thresholds FILE [--out FILE]` invokes the real public `preflight` and `route` interfaces over the frozen synthetic release fixture. It emits `megamind/benchmark-result/v1` with separate exact, near, paraphrase, ambiguous, no-match, and privacy metrics, authorized context accounting, canary/access safety counts, determinism, and local grep/full-vault baselines. The query set must carry a registered `megamind/benchmark-query-set/v1` frozen header, and the threshold file must carry a `[binding]` section naming the corpus, query-set, and task-set digests it gates; a missing, stale, or tampered identity is refused. A route candidate is only counted as loaded once `preflight` authorizes it for the declared model class, for every model class. `--repeat` requires byte-identical reruns. `bench check --results FILE --thresholds FILE` emits `megamind/benchmark-check/v1` and exits 1 when a frozen gate fails; a tier with no queries scores zero rather than passing. No model, network, or external service is used.

## megamind-axi experiment plan|validate|score|record

`experiment plan` freezes a versioned task set, rubric, thresholds, model/provider identifier, tools, effort, seed, and distinct no-wiki/current-wiki/updated-wiki snapshot roots. It creates opaque deterministic arm labels and isolated arm output roots, and writes three separate artifacts: the plan (`--out`), a grader packet with only blind identities and the rubric (`--grader-out`), and the host-only unblinding map (`--map-out`). The host executes identical tasks and supplies outputs; Megamind never invokes a worker or model. `validate --plan PLAN --outputs FILE...` needs no map and rejects missing tasks, duplicate arms, changed snapshot digests, malformed context accounting, prompt/canary leakage, model-access or privacy violations, and cross-arm contamination. `score --plan PLAN --outputs FILE... --unblinding-map FILE` seals the blind per-label scores before reading the map, then applies only the frozen rubric and promotion gates, returning `promoted` or `rollback-required`; an incomplete arm set is a typed `unsettled` document rather than a silent promotion. `record --score FILE --audit-root DIR` appends a bounded safe hash-chained event with a rollback reference and no prompt, answer, secret, or sensitive-content copy. Every output destination inside an evaluated root or any vault is refused.
