...
`experiment plan` freezes a versioned task set, rubric, thresholds, model/provider identifier, tools, effort, execution seed, and distinct no-wiki/current-wiki/updated-wiki snapshot roots. It takes the host's private blinding key from `--blinding-key-file` (at least 16 characters), creates opaque arm labels and isolated arm output roots, and writes three separate artifacts: the plan (`--out`), a grader packet with only blind identities, the rubric, and a keyed commitment (`--grader-out`), and the host-only unblinding map holding the key, the assignment, and the snapshot roots (`--map-out`). Neither public artifact carries enough to derive which label is which condition. The host executes identical tasks and supplies outputs; Megamind never invokes a worker or model. `validate --plan PLAN --outputs FILE...` needs no map or key and rejects missing tasks, duplicate arms, snapshots that do not cover the planned set exactly once, malformed context accounting, prompt/canary leakage, model-access or privacy violations, and cross-arm contamination. `score --plan PLAN --outputs FILE... --unblinding-map FILE` seals the blind per-label scores before reading the map, accepts the map only if it opens the plan's commitment under its own key, then applies only the frozen rubric and promotion gates, returning `promoted` or `rollback-required`; an incomplete arm set is a typed `unsettled` document rather than a silent promotion. `record --score FILE --audit-root DIR` appends a bounded safe hash-chained event with a rollback reference and no prompt, answer, secret, or sensitive-content copy. Every output destination inside an evaluated root - fixture, snapshot, output tree, or audit root - or any vault is refused before any work runs.
