...
`docs/schemas.md` (file formats), `docs/evaluation.md` (benchmark, three-arm,
and research-corpus evaluation contract), `docs/rollout.md` (host rollout proof
...
- `examples/vault/` is generated with Megamind's own APIs and pinned by
  ...
  in `evals/thresholds.toml`. The independent governed-research corpus is
  generated by `evals/gen_research_corpus.py` and fail-closed validated by
  `evals/research_benchmark.py`; its snapshots, labels, cases, mutations,
  manifest, and thresholds must be regenerated together. Regenerate any of
  these fixtures deliberately, never hand-drift one.
